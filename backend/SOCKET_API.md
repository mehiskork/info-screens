...
- Car numbers and driver names must be unique within a session
...
All core racing features and authentication roles are fully functional. You can:

- Create and manage sessions with manual car assignments (1–8)
- Authenticate receptionists, safety officials, and observers via handshake
- Start races, control race modes, and trigger the start-lights sequence
- Query next race queue with `upcoming`/`paddock` state
- End sessions after paddock return via `session:end`
- Record lap crossings and view real-time leaderboard sorted by best lap time
- Query and display all-time top-10 lap times (persisted across restarts)
- Receive full state snapshot on connect (no polling needed on load)
